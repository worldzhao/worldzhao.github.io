...
```
## 深克隆变形题目：将json字符串所有键名第一个首字母转为大写(考虑嵌套对象)
例：输入
```js
...
function toUpperCase1(obj) {
    ...
    for(var key in obj){
        var copy=obj[key];
        if(isClass(copy)=="Object" || isClass(copy)=="Array"){
            result[key.slice(0, 1).toUpperCase() +key.slice(1)]=arguments.callee(copy);
        }else{
...
## 事件委托
实现：给定一个ul列表，里面有若干个li标签，li里面也嵌套了若干标签，要求点击标签，弹出当前li在ul中的位置.
```html
<!DOCTYPE html>
<html lang="en">
<head>
